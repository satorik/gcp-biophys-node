...
const getUploadedLinks = async (
  ...
  const { file: filePath, imageUrl, fileLink } = await writeImage(
    uploadedFile,
    currentSection,
    fileUploadName,
    fileType
...
const valueForCreateImg = async (
  ...
  if (fileType === 'image')
    return getCreateDataImage(postData, uploaded.imageUrl, user)
  else if (fileType === 'pdf')
    return getCreateDataPDF(postData, uploaded.fileLink, user, uploaded.image)
  else {
    throw new ApolloError('File Type not recognized')
  }
}
...
const valueForUpdateImg = async (
  ...
  const post =
    currentSection === 'history' || currentSection === 'admission'
