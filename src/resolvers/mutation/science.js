...
import {
  ...
  valueForUpdateImg,
  valueForDeleteImg,
} from '../../utils/getMutationValue'

const scienceMutation = {
  async createScienceGroup(
    parent,
    { scienceRouteId, inputData },
    { models, auth }
  ) {
    const scienceRoute = await models.ScienceRoute.findOne({
      where: { id: scienceRouteId },
    })
    if (!scienceRoute) {
      throw new ApolloError(`Science Route with id ${scienceRouteId} not found`)
    }

    const { createData, imageUrl } = await valueForCreateImg(
      inputData,
      'scienceGroup',
      auth,
      models.User
    ...
    return createWithImage(
      models.ScienceGroup,
      { ...createData, scienceRouteId },
      imageUrl,
      'scienceGroup'
    )
  },
  async updateScienceGroup(parent, { id, inputData }, { models, auth }) {
    const { updateData, imageToClear, isUploaded } = await valueForUpdateImg(
      id,
      inputData,
      'scienceGroup',
      auth,
  ...
  async deleteScienceGroup(parent, { id }, { models, auth }) {
    const forDelete = await valueForDeleteImg(
      auth,
      models.User,
      id,
      models.ScienceGroup
    )
    return deleteWithImage(
      forDelete,
      forDelete.dataValues.imageUrl,
      'scienceGroup',
      id
    )
  },
  async createScienceRoute(parent, { inputData }, { models, auth }) {
    ...
    const scienceRoute = await models.ScienceRoute.create({
      ...inputData,
      userCreated: user,
    })
    return scienceRoute
  ...
  async updateScienceRoute(parent, { id, inputData }, { models, auth }) {
    ...
    if (!route) {
      throw new ApolloError('Route not found')
    }
  ...
  async createSciencePerson(
    parent,
    { scienceGroupId, inputData },
    { models, auth }
  ) {
    const user = await getUser(auth, models.User)

    const scienceGroup = await models.ScienceGroup.findOne({
      where: { id: scienceGroupId },
    })
    if (!scienceGroup) {
      throw new ApolloError(`Science Group with id ${scienceGroupId} not found`)
    }

    const peopleCount = await models.SciencePeople.count({
      where: { scienceGroupId },
    })
  ...
  async updateSciencePerson(parent, { id, inputData }, { models, auth }) {
    ...
    if (!person) {
      throw new ApolloError('person with id ${id} not found')
    }
  ...
  async deleteSciencePerson(parent, { id }, { models, auth }) {
    ...
    const people = await models.SciencePeople.findAll({
      where: { scienceGroupId: person.scienceGroupId },
      order: [['position', 'ASC']],
    })
  ...
  async createScienceArticle(
    ...
    const articleCount = await models.ScienceArticle.count({
      where: { scienceGroupId },
    })
  ...
  async updateScienceArticle(parent, { id, inputData }, { models, auth }) {
    ...
    if (!article) {
      throw new ApolloError('Article with id ${id} not found')
    }
  ...
  async deleteScienceArticle(parent, { id }, { models, auth }) {
    ...
    const articles = await models.SciencePeople.findAll({
      where: { scienceGroupId: person.scienceGroupId },
      order: [['position', 'ASC']],
    })
  ...
  async moveSciencePerson(parent, { id, vector }, { models, auth }) {
    ...
    if (vector === 'UP') {
      if (person.position === 0)
        throw new ApolloError('Person can not go any higher')
    ...
    if (vector === 'DOWN') {
      ...
      if (person.position === peopleCount - 1)
        throw new ApolloError('Person can not go any lower')
      newPosition = person.position + 1
    }

    if (newPosition < 0) throw new ApolloError('Something wrong')

    const personToMove = await models.SciencePeople.findOne({
      where: { scienceGroupId: person.scienceGroupId, position: newPosition },
    })
    if (!personToMove)
      throw new ApolloError(
        'person with scienceGroupId ${person.scienceGroupId} and position ${newPosition} not found'
      )
  ...
  async moveScienceArticle(parent, { id, vector }, { models, auth }) {
    ...
    if (vector === 'UP') {
      if (article.position === 0)
        throw new ApolloError('article can not go any higher')
      newPosition = article.position - 1
    }
    if (vector === 'DOWN') {
      const articleCount = await models.ScienceArticle.count({
        where: { scienceGroupId: article.scienceGroupId },
      })
      if (article.position === articleCount - 1)
        throw new ApolloError('article can not go any lower')
      newPosition = article.position + 1
    }

    if (newPosition < 0) throw new ApolloError('Something wrong')

    const articleToMove = await models.ScienceArticle.findOne({
      where: { scienceGroupId: article.scienceGroupId, position: newPosition },
    })
    if (!articleToMove)
      throw new ApolloError(
        'article with scienceGroupId ${article.scienceGroupId} and position ${newPosition} not found'
      )
