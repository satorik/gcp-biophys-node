...
const updateWithImage = async (
  ...
  try {
    ...
    if (imageToClear) {
      await clearImage(imageToClear, currentSection, fileType)
    }
    await t.commit()
    return post
  } catch (error) {
    await t.rollback()
    if (upload) {
      await clearImage(upload.imageUrl, currentSection, fileType)
    }
    throw error
  }
}

const deleteWithImage = async (
  post,
  imageUrl,
  currentSection,
  id,
  ...
  try {
    ...
    await clearImage(imageUrl, currentSection, fileType)
    await t.commit()
    return id
  } catch (error) {
    console.log(error)
